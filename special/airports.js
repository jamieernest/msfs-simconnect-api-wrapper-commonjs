...
exports.AirportHandler = exports.AirportEvents = void 0;
exports.getAirportHandler = getAirportHandler;
exports.getHeadingFromTo = getHeadingFromTo;
exports.loadAirportDB = loadAirportDB;
...
const _dirname = _nodeUrl.default.fileURLToPath(new URL(".", require('url').pathToFileURL(__filename).toString()));
const AIRPORT_DB_LOCATION = _nodePath.default.join(_dirname, `..`, `airport.db.gz`);
const AirportEvents = exports.AirportEvents = {
  AIRPORTS_IN_RANGE: {
    name: `AirportsInRage`,
...
const AIRPORT_TYPE = _constants.SIMCONNECT_FACILITY_LIST_TYPE_AIRPORT;
 ...
 * ...docs go here...
 */
async function getAirportHandler(api, handle) {
  const TYPE = _constants.SIMCONNECT_FACILITY_LIST_TYPE_AIRPORT;
  ...
  handle.on(`airportList`, ({
    ...
    if (id === IN_RANGE || id === OUT_OF_RANGE) {
      const eventName = AirportEvents[id === IN_RANGE ? `AIRPORTS_IN_RANGE` : `AIRPORTS_OUT_OF_RANGE`].name;
      api.eventListeners[eventName]?.handlers.forEach(handle => handle(airports));
    }
  });
...
function loadAirportDB(location = AIRPORT_DB_LOCATION) {
  if (cachedAirportData) return cachedAirportData;
  if (_nodeFs.default.existsSync(location)) {
    let zipped, json;
    try {
      zipped = _nodeFs.default.readFileSync(location);
    } catch (e) {
      console.error(`file read error:`, e);
    }
    try {
      json = _nodeZlib.default.gunzipSync(zipped);
    } catch (e) {
...
class AirportHandler {
  ...
  async init(remote, finished) {
    ...
    const {
      handle
    } = await (0, _nodeSimconnect.open)(`airport`, _nodeSimconnect.Protocol.KittyHawk, remote);
    this.handle = handle;
    ...
    if (this.airports) {
      ...
      if (this.airports.length !== airportCount) {
        console.warn(`MSFS has ${airportCount} airports, db has ${this.airports.length} airports.`);
        console.warn(`You may need to update your version of msfs-simconnet-api-wrapper...`);
      }
      return finished();
    }
    ...
    const zipped = _nodeZlib.default.gzipSync(Buffer.from(json));
    _nodeFs.default.writeFileSync(AIRPORT_DB_LOCATION, zipped);
    finished();
  }
  ...
  async get(varName) {
    ...
    if (varName === ALL_AIRPORTS) {
      return {
        [propName]: this.airports
      };
    }
    if (varName.startsWith(NEARBY_AIRPORTS)) {
      return {
        [propName]: await this.getAllNearbyAirports(varName)
      };
    }
    if (varName.startsWith(AIRPORT)) {
      return {
        [propName]: await this.getAirport(varName)
      };
    }
    return {};
  }
  ...
  async getAllNearbyAirports(varName) {
    ...
    } = await this.api.get(`PLANE_LATITUDE`, `PLANE_LONGITUDE`);
    ...
    this.airports.forEach(airport => {
      ...
      const d = (0, _utils.getDistanceBetweenPoints)(lat, long, alat, along);
      if (d <= KM) found.push({
        ...airport,
        distance: d / KM_PER_NM
      });
    });
 ...
 * @returns
 */
exports.AirportHandler = AirportHandler;
...
function addAirportRunway(data, airportData, airport) {
  ...
  const surface = _constants.RUNWAY_SURFACES[data.readInt32()];
  const primary_number = _constants.RUNWAY_NUMBER[data.readInt32()];
  const primary_designator = _constants.RUNWAY_DESIGNATOR[data.readInt32()];
  const primary_ils_type = _constants.ILS_TYPES[data.readInt32()];
  const primary_ils_icao = data.readString8();
  const primary_ils_region = data.readString8();
  const secondary_number = _constants.RUNWAY_NUMBER[data.readInt32()];
  const secondary_designator = _constants.RUNWAY_DESIGNATOR[data.readInt32()];
  const secondary_ils_type = _constants.ILS_TYPES[data.readInt32()];
...
function setRunwayBounds(runway) {
  ...
  runway.bbox = [[lat1, long1], [lat2, long2], [lat3, long3], [lat4, long4]];
}
...
function getHeadingFromTo(lat1, long1, lat2, long2, declination = 0) {
